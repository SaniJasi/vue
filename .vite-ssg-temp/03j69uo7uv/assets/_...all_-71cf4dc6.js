...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if ($setup.todo) {
    _push(
      `<div class="single-todo" data-v-e9f0bf08><div class="single-todo__title" data-v-e9f0bf08>`
    )
    if (!$setup.editItem) {
      _push(`<h1 data-v-e9f0bf08>${ssrInterpolate($setup.name)}</h1>`)
    } else {
      _push(
        `<input${ssrRenderAttr('value', $setup.newName)}${ssrRenderAttr(
          'placeholder',
          $setup.name
        )} data-v-e9f0bf08>`
      )
    ...
    if ($setup.todo && $setup.todo.item) {
      ...
      ssrRenderList($setup.todo.item.todos, (field, index) => {
        _push(
          `<li data-v-e9f0bf08><input type="checkbox"${
            ssrIncludeBooleanAttr(
    ...
    _push(
      `<div class="todo-controls" data-v-e9f0bf08><button class="btn btn--red" data-v-e9f0bf08>Remove</button><button class="btn btn--secondary" data-v-e9f0bf08>${ssrInterpolate(
        $setup.editItem ? 'Cancel' : 'Edit'
      )}</button><button class="btn btn--primary" data-v-e9f0bf08>Save</button></div></div>`
    )
  } else {
    _push(`<!---->`)
  }
      ...
      },
      {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            return [createVNode('p', null, 'Are you sure you want to delete this item?')]
          }
        }),
        _: 1
        /* STABLE */
...
const ____all_ = /* @__PURE__ */ _export_sfc(_sfc_main, [
  ['ssrRender', _sfc_ssrRender],
  ['__scopeId', 'data-v-e9f0bf08'],
  ['__file', '/Users/a51.hs/Desktop/vue-todo/src/pages/[...all].vue']
])
